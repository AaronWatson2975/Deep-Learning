...
ALL_MODELS = sum((conf for conf in (GPT2Config.get_config_dict(
    "gpt2"), OpenAIGPTConfig.get_config_dict("openai-gpt"))), ())
        ...
        for _ in trange(length):
            ...
            if is_xlm_mlm and xlm_mask_token:
                ...
                input_ids = torch.cat((generated, torch.full(
                    (1, 1), xlm_mask_token, dtype=torch.long, device=device)), dim=1)
                inputs = {'input_ids': input_ids}

            if xlm_lang is not None:
                inputs["langs"] = torch.tensor(
                    [xlm_lang] * inputs["input_ids"].shape[1], device=device).view(1, -1)
...
def main():
    ...
    while True:
        ...
        is_xlm_mlm = args.model_type in [
            "xlm"] and 'mlm' in args.model_name_or_path
        if is_xlm_mlm:
